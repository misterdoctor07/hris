        ...
        <div class="tab-content">
            ...
            while ($company = mysqli_fetch_array($sqlCompanies)) {
                ...
                while ($department = mysqli_fetch_array($sqlDepartments)) {
                    ...
                    $sqlEmployee = mysqli_query($con, "SELECT ep.*, ed.*, er.*, d.department, ed.designation, jt.jobtitle, er.referredby AS refID, CONCAT(ref.firstname, ' ', ref.lastname) AS referrer_name
                        FROM employee_profile ep
                        INNER JOIN employee_details ed ON ed.idno = ep.idno
                        INNER JOIN department d ON d.id = ed.department
                        INNER JOIN jobtitle jt ON jt.id = ed.designation
                        INNER JOIN employee_referral er ON er.idno = ep.idno
                        LEFT JOIN employee_profile ref ON ref.idno = er.referredby
                        WHERE ed.company = '$companyCode'
                    ...
                    while ($employee = mysqli_fetch_array($sqlEmployee)) {
                        ...
                        $referrerDetails = $employee['referrer_name']." (".$employee['refID'].")";
                        $referralfee = date('M d, Y', strtotime($employee['effectivity']));
                        if ($employee['refID'] == '' || $employee['refID'] == "N/A"){
                            $referrerDetails = "";
                            $referralfee = "";
                        }
