...
if (isset($_GET['submit'])) {
  ...
  if (!empty($newStatus)) {
      ...
      if ($sqlInsert) {
          echo "<script>alert('Attendance successfully saved!'); window.location='?edittime&idno=$idno&logindate=$logindate&id=$id&company=$comp';</script>";
      } else {
          echo "<script>alert('Unable to save details!'); window.location='?edittime&idno=$idno&logindate=$logindate&id=$id&company=$comp';</script>";
      }
  } else {
      echo "<script>alert('Please select at least 1 status!'); window.location='?edittime&idno=$idno&logindate=$logindate&id=$id&company=$comp';</script>";
  }
}
