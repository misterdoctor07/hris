...
if(isset($_GET['token'])) {
    ...
    if(mysqli_num_rows($result) > 0) {
        ...
        if($data['is_valid'] && !$data['already_used']) {
            // Mark token as used
            $update = mysqli_prepare($con, "UPDATE payslip_tokens SET is_used = TRUE, accessed_at = NOW() WHERE token = ?");
            ...
            $sqlEmployee = mysqli_prepare($con, "SELECT * FROM employee_profile ep
                INNER JOIN employee_details ed ON ep.idno=ed.idno
                INNER JOIN department d ON d.id=ed.department
                WHERE ep.idno=?");
            mysqli_stmt_bind_param($sqlEmployee, "s", $idno);
            mysqli_stmt_execute($sqlEmployee);
            $employeeResult = mysqli_stmt_get_result($sqlEmployee);
    ...
    $totaldeductAmount = number_format($totaldeductions,2);
            ...
            error_log("Access denied - Token expired or already used");
