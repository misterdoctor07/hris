            ...
            <?php
                ...
                while ($company = mysqli_fetch_array($sqlCompanies)) {
                    ...
                    $sqlCount = mysqli_query($con, "SELECT COUNT(*) AS total FROM missed_log_application ml
                        ...
                        AND (monitoring_remarks IS NULL OR monitoring_remarks = '')"); 
...
if (isset($_GET['addremarks'])) {
    $id = $_GET['id'];
    $remarks = urldecode($_GET['monitoring_remarks']); // Use urldecode to handle special characters
?>
    <!-- Remarks Form -->
    ...
    <div class="modal-container">
        <div class="content-panel">
            <div class="panel-heading-">
                <h4>
                    <a href="?monitoringmanagemissedlogapplication"><i class="fa fa-arrow-left"></i> Close</a> |
...
if (isset($_POST['submitRemarks'])) {
    ...
    if (mysqli_query($con, $sqlUpdateRemarks)) {
        ...
        echo "<script>window.location.href='?monitoringmanagemissedlogapplication';</script>";
    } else {
        echo "<script>alert('Error updating remarks: " . mysqli_error($con) . "');</script>";
    }
}
...
function filterByDate() {
    ...
    if (fromDate && toDate) {
            window.location.href = `?monitoringmanagemissedlogapplication&fromDate=${fromDate}&toDate=${toDate}`;
        } else {
