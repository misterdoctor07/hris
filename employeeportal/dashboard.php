            ...
            <aside>
                <div id="sidebar" class="nav-collapse ">
                    <ul class="sidebar-menu" id="nav-accordion">
                        ...
                        <?php if ($idno == '111111' || $idno == '102215'):?>
                            <?php
                                $sqlNurseNote = mysqli_query($con, "SELECT COUNT(*) AS total 
                                                                    FROM wfh_application
                                                                    WHERE reason_type = 'Medical'
                                                                    AND (nurse_note = '' OR nurse_note IS NULL)");
                                $note = mysqli_fetch_array($sqlNurseNote);
                                $note_count = $note['total'];
                            ?>
                            <li>
                        ...
                        <li>
                            <?php if ($designation == 8||$designation == 50||$designation == 89 || $designation == 59|| $designation == 65|| $designation == 115 || $designation == 94||$designation == 102 || $designation == 3 || $designation == 88|| $designation == 114||$designation == 92 || $userId == '103417'): ?>
                        ...
                        <?php endif; ?>
                        <?php if ($designation == 97 || $designation == 76 || $designation == 65 || $designation == 50|| $designation == 89 || $designation == 93 || $designation == 59 || $designation == 94 || $designation == 115 || $designation == 114 || $designation == 92 ): ?>
                            <li>
                                <a href="dashboard.php?empbreakdownpoints">
                                    <i class="fa fa-bar-chart" aria-hidden="true"></i>
                                    <span>Points Tracker</span>
                                </a>
                            </li>
                        <?php endif; ?>
                      <?php if ($designation == 93 || $designation == 97): ?>
                            <li>
                                <a href="dashboard.php?contact_person">
                ...
                <section class="wrapper site-min-height">
                    ...
                    <div class="row mt">
                        <?php
                            ...
                            if(isset($_GET['movementApp'])){include('movementApp.php');}
                            if(isset($_GET['addmovementemp'])){include('addmovementemp.php');}
                            if(isset($_GET['empbreakdownpoints'])){include('empbreakdownpoints.php');}
                            if(isset($_GET['handbook'])){include('handbook.php');}
                            ...
                            if(isset($_GET['editempmovement'])){include('editempmovement.php');}
                            if(isset($_GET['manage_wfhapplications'])){include('manage_wfhapplications.php');}
                            if(isset($_GET['transfer_form'])){include('transfer_form.php');}
                            if(isset($_GET['addtransfer'])){include('addtransfer.php');}
                            if(isset($_GET['edit_transferform'])){include('edit_transferform.php');}
                            if(isset($_GET['manage_transferapplication'])){include('manage_transferapplication.php');}
                            if(isset($_GET['terms'])){include('terms.php');}
                            if(isset($_GET['allow_employee'])){include('allow_employee.php');}
                            if(isset($_GET['update_wfh_status'])){include('update_wfh_status.php');}
            ...
            $(document).ready(function() {
              $.ajax({
                ...
                success: function(data) {
                            ...
                            //For WFH Request Count
                            if (data.wfh_req_count !== undefined) {
                                if (data.wfh_req_count > 0) {
                                    $('#wfh-notification-badge, #wfh-req-notification-badge').html(data.wfh_req_count).show();
                                } else {
                                    $('#wfh-notification-badge, #wfh-req-notification-badge').hide(); // Hide if count is 0
                                }
                            } else {
                                console.error('WFH count missing in response.');
                            }
                            //For WFH Ack Count
                            if (data.wfh_ack_count !== undefined) {
                                if (data.wfh_ack_count > 0) {
                                    $('#wfh-it-notification-badge').html(data.wfh_ack_count).show();
                                } else {
                                    $('#wfh-it-notification-badge').hide(); // Hide if count is 0
                                }
                            } else {
                                console.error('WFH count missing in response.');
                            }
                            //For Transfer Ack Count
                            if (data.transfer_count !== undefined) {
                                if (data.transfer_count > 0) {
                                    $('#transfer-it-notification-badge').html(data.transfer_count).show();
                                } else {
                                    $('#transfer-it-notification-badge').hide(); // Hide if count is 0
                                }
                            } else {
                                console.error('Transfer count missing in response.');
                            }
                            //Total
