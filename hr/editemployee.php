<?php
$id=$_GET['id'];
$sqlProfile=mysqli_query($con,"SELECT ep.*,ed.* FROM employee_profile ep LEFT JOIN employee_details ed ON ed.idno=ep.idno WHERE ep.id='$id'");
...
$dateregular=$profile['dateofregular'];
$dateoffulltime=$profile['dateoffulltime'];
$dateleaveeffective=$profile['dateleaveeffective'];
...
if(mysqli_num_rows($sqlChecklist)>0){
    $checklist=mysqli_fetch_array($sqlChecklist);
    $referredby=$checklist['referredby'];
    $referreddate=$checklist['effectivity'];
    $sqlRef=mysqli_query($con,"SELECT * FROM employee_profile WHERE idno='$referredby'");
    if(mysqli_num_rows($sqlRef)>0){
      $ref=mysqli_fetch_array($sqlRef);
      $referredbyname=$ref['lastname'].", ".$ref['firstname'];
    }else{
      $referredbyname="";
    }
}else{
    ...
    <div class="col-lg-4 mt">
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-9">
                    <input type="text" class="form-control" name="contactperson" required value="<?=$contactperson;?>">
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-3 col-sm-3 control-label">Contact Number</label>
                  <div class="col-sm-4">
                    <input type="text" class="form-control" name="contactnumber" required value="<?=$contactnumber;?>">
                  </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-3 col-sm-3 control-label">Relationship</label>
                  <div class="col-sm-5">
                    <input type="text" class="form-control" name="relationship" required value="<?=$relationship;?>">
                  </div>
                </div>
            </div>
          </div>
        ...
        <div class="col-lg-4 mt">
            ...
            <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-5">
                        <select name="department" class="form-control" required>
                            ...
                            if(mysqli_num_rows($sqlCompany)>0){
                                while($comp=mysqli_fetch_array($sqlCompany)){
                                    // Check if the current department matches the option
                                    $selected = ($comp['id'] == $deptid) ? 'selected' : '';
                                    echo "<option value='$comp[id]' $selected>$comp[department]</option>";
                                }
                            }
                            ?>
                        </select>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <select name="jobtitle" class="form-control" required>
                            ...
                            if(mysqli_num_rows($sqlCompany)>0){
                                while($comp=mysqli_fetch_array($sqlCompany)){
                                    $selected = ($comp['id'] == $jobid) ? 'selected' : '';
                                    echo "<option value='$comp[id]' $selected>$comp[jobtitle]</option>";
                                }
                            }
                            ?>
                        </select>
                ...
                <div class="form-group">
                  <label class="col-sm-3 col-sm-3 control-label">Date of Regularization</label>
                  <div class="col-sm-5">
                    <input type="date" name="dateregular" class="form-control" required value="<?=$dateregular;?>">
                  </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-3 col-sm-3 control-label">Leave Effectivity</label>
                    <div class="col-sm-5">
                        <input type="date" name="dateleaveeffective" class="form-control" required value="<?=$dateleaveeffective;?>">
                    </div>
                </div>
                  <div class="form-group">
                ...
                <div class="form-group">
                    <label class="col-sm-3 col-sm-3 control-label">Branch Location</label>
                    <div class="col-sm-5">
                        <select name="work_area" class="form-control" required >
                       <option value="<?=$work_area;?>">
                        <?php
                        if($work_area=="Davao"){
                          echo "Davao City";
                        } elseif ($work_area=="Iloilo"){
                          echo "Iloilo City";
                        } elseif ($work_area=="Kidapawan"){
                          echo "Kidapawan City";
...
<!--                <select name="referredby" class="form-control" required>-->
                    <input type="text" list="refer" class="form-control" name="referredby" value="<?=$referredbyname;?>_<?=$referredby;?>">
                    <datalist id="refer">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-5">
                  <input type="date" class="form-control" name="referreddate" value="<?=$referreddate;?>">
                </div>
              </div>
          </div>
        </div>
  ...
  <?php
    if(isset($_GET['submit'])){
      ...
      $dateleaveeffective=$_GET['dateleaveeffective'];
      ...
      $phic=$_GET['phic'];
      $hdmf=$_GET['hdmf'];
      $sqlBenefitsCheck=mysqli_query($con,"SELECT * FROM employee_benefits WHERE idno='$idno'");
      if(mysqli_num_rows($sqlBenefitsCheck)>0){
        ...
        $values="VALUES('$idno','$insurance','$hmo','$sss','$tin','$phic','$hdmf','$updatedby','$datenow')";
      ...
      $dateoriented=$_GET['dateoriented'];
        ...
        if(mysqli_num_rows($sqlCheck)>0){
            $referredby=explode('_',$referredby);
            $table="employee_referral";
            $values="SET referredby='$referredby[1]',effectivity='$referreddate',updatedby='$addedby',updateddatetime='$datenow' WHERE idno='$idno'";
            $sqlAddEmployee=mysqli_query($con,"UPDATE $table $values");
        }else{
