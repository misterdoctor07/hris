                ...
                <div class="col-lg-12">
                    <div class="content-panel">
                        <div class="panel-heading" style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <div>
                                <a href="?view_eo_credits" type="button" class="btn btn-primary">VIEW MONTHLY EO CREDITS</a>
                            </div>
                        </div>
                        ...
                        <div class="tab-content">
                            ...
                            while ($company = mysqli_fetch_array($sqlCompanies)) {
                                            ...
                                            $sqlEmployee = mysqli_query($con, "SELECT ep.*, ed.*, d.department, ed.designation, jt.jobtitle 
                                             ...
                                             if(mysqli_num_rows($sqlEmployee)>0){
                                                while($company=mysqli_fetch_array($sqlEmployee)){
                                                  ...
                                                  $dateregular=date('M d, Y',strtotime($company['dateofhired'])); // dria mag update
                                                  $dateofregular=date('M d, Y',strtotime(strtotime($company['dateofregular']))); 
                                                  $dateofhired=date('M d, Y',strtotime(strtotime($company['dateofhired'])));
                                                  $dateleaveeffective=date('M d, Y',strtotime(strtotime($company['dateleaveeffective'])));
                                                  ...
                                                  // Assuming $company['dateofhire'] is in 'Y-m-d' format
                                                    ...
                                                    $thresholdDate = new DateTime('2020-07-31'); // End of July 2020
                                                     ...
                                                     // Get hire date based on your threshold logic
                                                    ...
                                                    // Set period dates (unchanged from your code)
                                                    $periodfrom = date('M d, Y', strtotime($years . " years", strtotime($company['dateofregular'] ?? $company['dateofhired'])));
                                                    $periodto = date('M d, Y', strtotime('1 years', strtotime($periodfrom)));
                                                    
                                                    // Check if today is anniversary
                                                    $currentMonth = date('n');
                                                    $currentDay = date('j');
                                                    $dateHiredMonth = $dhire->format('n');
                                                    $dateHiredDay = $dhire->format('j');
                                                    
                                                    if ($currentMonth == $dateHiredMonth && $currentDay == $dateHiredDay) {
                                                        // Check when we last reset credits for this employee
                                                        $result = mysqli_query($con, "SELECT last_reset_date FROM leave_credits WHERE idno = '$company[idno]'");
                                                        $row = mysqli_fetch_assoc($result);
                                                        $lastResetDate = $row['last_reset_date'] ?? null;
                                                        
                                                        // Only reset if we haven't done it this year
                                                        if (!$lastResetDate || date('Y', strtotime($lastResetDate)) != date('Y')) {
                                                            // Reset leave credits and update last reset date
                                                            $sqlResetCredits = mysqli_query($con, "
                                ...
                                $sqlCheckCredits = mysqli_query($con, "SELECT * FROM leave_credits WHERE idno = '$company[idno]'");
                                        ...
                                        $vlrem = $slrem = $ptorem = 0;
                                        $sqlChecklist=mysqli_query($con,"SELECT * FROM leave_credits WHERE idno='$company[idno]'");
                                        if(mysqli_num_rows($sqlChecklist)>0){
                                            $checklist=mysqli_fetch_array($sqlChecklist);
                                            $leavecredits = $checklist['vacationleave'];
                                            $leaveused = $checklist['vlused'];
                                            $vlrem = $leavecredits - $leaveused;
                                            $sickleaves = $checklist['sickleave'];
                                            $sickused = $checklist['slused'];
                                            $slrem = $sickleaves - $sickused;
                                            $ptocredits = $checklist['pto'];
                                            $ptoused = $checklist['ptoused'];
                                ...
                                <tr>
                                    ...
                                    <?php
                                    echo "<td align='center' width='1%'>$x.</td>";
                                    echo "<td width='4%' align='center'>$company[idno]</td>";
                                    echo "<td width='17%'><strong>$company[lastname]</strong>, $company[firstname] $company[middlename] $company[suffix]</td>";
                                    echo "<td width='6%' align='center'>$dateregular</td>"; // dria mag base pag update date of hire
                                    echo "<td width='12%' style='padding-left: 20px;'>$years years $month months $days days</td>";
                                    echo "<td width='11%' align='center'>$periodfrom - $periodto </td>";
                                    echo "<td align='center' style='background-color: #FFFACD'>{$leavecredits}</td>";
                                    echo "<td align='center' class='used-leave' data-leave-type='VL' data-employee-id='{$company['idno']}'>{$leaveused}</td>";
                                    echo "<td align='center' style='background-color: $vlcolor;'>{$vlrem}</td>";
                                    echo "<td align='center' style='background-color: #FFFACD'>{$sickleaves}</td>";
                                    echo "<td align='center' class='used-leave' data-leave-type='SL' data-employee-id='{$company['idno']}'>{$sickused}</td>";
                                    echo "<td align='center' style='background-color: $slcolor;'>{$slrem}</td>";
                                    echo "<td align='center' style='background-color: #FFFACD'>{$ptocredits}</td>";
                                    echo "<td align='center' class='used-leave' data-leave-type='PTO' data-employee-id='{$company['idno']}'>{$ptoused}</td>";
                                    echo "<td align='center' style='background-color: $ptocolor;'>{$ptorem}</td>";
                                echo "</tr>";
                                $x++;
                                }
                            }else{
                ...
                $active = ''; // Remove active class for subsequent companies
                        ...
                        <script>
                         ...
                         $(document).ready(function() {
                            // Context menu for leave dates
                            $('.used-leave').on('contextmenu', function(e) {
                                e.preventDefault();
                                
                                const leaveType = $(this).data('leave-type');
                                const employeeId = $(this).data('employee-id');
                                
                                // Fetch leave dates via AJAX
                                $.ajax({
                                    ...
                                    success: function(response) {
                                        // Create and show a popup with the dates
                                        const popup = $('<div class="leave-dates-popup"></div>')
                                            .css({
                                                'position': 'absolute',
                                                'left': e.pageX + 'px',
                                                'top': e.pageY + 'px',
                                            ...
                                            .html('<h4>' + leaveType + ' Dates</h4>' + response)
                                            .appendTo('body');
                        ...
                        .leave-dates-popup ul {
                            margin: 0;
                            padding: 0;
                            list-style: none;
                        }
                        
                        .leave-dates-popup li {
                            padding: 5px;
                            border-bottom: 1px solid #eee;
                        }
                        
                        .leave-dates-popup li:last-child {
                            border-bottom: none;
                        }
                        </style>
